"""SQL Server → MinIO landing as Parquet (streaming batches)."""
...
def transfer_table_to_parquet(
    ...
    """Read SQL Server table and write Parquet to MinIO. Path: s3://{bucket}/{schema}/{table}/data.parquet"""
    ...
    hook = MsSqlHook(mssql_conn_id=mssql_conn_id, schema=schema)
    conn = hook.get_conn()
    cursor = conn.cursor()
